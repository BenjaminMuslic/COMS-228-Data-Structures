...
public abstract class Living {
	...
	protected void census(int population[]) {
		...
		int left = plain.getWidth();
		int right = plain.getWidth();

		for (int plain_length = Math.max(0, row - 1); plain_length <= Math.min(row + 1, left - 1); plain_length++) {
			for (int plain_width = Math.max(0, column - 1); plain_width <= Math.min(column + 1,
					right - 1); plain_width++) {

				if (plain.grid[plain_length][plain_width].who() == State.BADGER) {
					population[BADGER]++;
				} else if (plain.grid[plain_length][plain_width].who() == State.EMPTY) {
					population[EMPTY]++;
				} else if (plain.grid[plain_length][plain_width].who() == State.FOX) {
					population[FOX]++;
				} else if (plain.grid[plain_length][plain_width].who() == State.GRASS) {
					population[GRASS]++;
				} else if (plain.grid[plain_length][plain_width].who() == State.RABBIT) {
					population[RABBIT]++;
				}
